...
public class edPortalAd {
    public edPortalAd(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("portaladvancement")
                .requires((source) -> source.hasPermission(2))
                .then(Commands.literal("everdawn")
                        .then(Commands.argument("advancement", ResourceLocationArgument.id())
                                .executes((command) -> {
                                    ...
                                    if (advancement != null) {
                                        ...
                                        commonConfig.everdawnPortalAdvancement.set(advancementId.toString());
                                        commonConfig.SPEC.save();
                                        command.getSource().sendSystemMessage(Component.literal("Everdawn portal will be enabled after getting advancement: " + advancementId).withStyle(ChatFormatting.AQUA));
                                        return 1;
                                    } else {
